...
def main():
    # 서버의 IP 주소와 포트 번호
    SERVER_HOST = '192.168.10.8' #'192.168.0.42'(실습실)/ '192.168.10.8'(시연장)  서버의 IP 주소
    ...
    for message in consumer:
        ...
        # 클라이언트로부터 메시지를 수신하거나 Enter 키 입력을 통해 처리
        while sorted_pkg_ids:
            print("Press Enter to send the next package ID or type 'exit' to quit.")
            data = input()
            if data.lower() == 'exit':
                print("Exiting...")
                return
