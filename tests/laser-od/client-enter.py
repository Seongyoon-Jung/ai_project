...
def main():
    # 서버의 IP 주소와 포트 번호
    SERVER_HOST = '192.168.0.25' #'192.168.0.42'(실습실)/ '192.168.10.4'(시연장)  서버의 IP 주소
    ...
    for message in consumer:
        ...
        # 클라이언트로부터 passed 메시지를 수신하거나 엔터 키를 감지하여 처리
        while sorted_pkg_ids:
            # 비동기 입력 감지
            input_ready, _, _ = select.select([sys.stdin, client_socket], [], [])
