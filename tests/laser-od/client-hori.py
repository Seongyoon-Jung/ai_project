...
# Define points for a horizontal line at the bottom third of the frame
line_y = 2 * h // 3  # Horizontal line at 2/3 height of the frame
line_points = [(0, line_y), (w, line_y)]  # Horizontal line coordinates
...
while cap.isOpened():
    ...
    # Draw confidence scores on the image
    for result in results:
        if result.boxes is not None:
            for box in result.boxes:
                ...
                if confidence >= 0.55:  # Only consider objects with confidence >= 0.5
                    bbox = box.xyxy[0].cpu().numpy()  # Ensure bbox is a list of values
